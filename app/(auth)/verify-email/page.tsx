...
import { Loader2, Zap, Instagram, BarChart3, ArrowRight, ShieldCheck } from 'lucide-react';
...
function CheckInboxCard({ email }: { email: string }) {
  return (
    <div className='w-full max-w-[480px] rounded-2xl border border-border bg-card p-12 text-center shadow-xl shadow-black/40'>
      {/* Envelope icon */}
      ...
      <p className='mt-5 text-sm text-muted-foreground leading-relaxed px-2'>
        We&apos;ve sent a verification link to your email. Click the link to activate your account and get 500 free credits to start automating.
      </p>
      ...
      <div className='mt-4'>
        <Link href='/login' className='inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors'>
          <svg className='h-4 w-4' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'><path d='M19 12H5m7-7l-7 7 7 7'/></svg>
          Back to login
        </Link>
      </div>
    </div>
...
function EmailVerifiedCard() {
  return (
    <div className='w-full max-w-[480px] rounded-2xl border border-success/25 bg-[#0d1f14] p-12 text-center shadow-xl shadow-black/40'>
      {/* Green checkmark */}
      ...
      <p className='mt-2 text-sm text-muted-foreground'>Your account is now active and ready to go.</p>

      {/* Credits banner */}
      <div className='mt-6 flex items-center gap-3 rounded-xl border border-success/20 bg-success/10 px-5 py-4 text-left'>
        <svg className='h-5 w-5 shrink-0 text-success' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
        ...
        <div>
          <p className='text-sm font-semibold text-success'>500 Free Credits Added!</p>
          <p className='text-xs text-muted-foreground mt-0.5'>Use them to power your first automations</p>
        </div>
      </div>
      ...
      <div className='mt-5 grid grid-cols-3 gap-3'>
        {[
          ...
          <div key={label} className='rounded-xl border border-border bg-card p-4 flex flex-col items-center gap-2'>
      ...
      <Button asChild className='mt-6 w-full h-10 bg-primary hover:bg-primary-hover text-white font-semibold rounded-[--radius-md]'>
        <Link href='/login'>
          <ArrowRight className='mr-2 h-4 w-4' />Continue to Login
        </Link>
      </Button>

      <p className='mt-5 flex items-center justify-center gap-1.5 text-xs text-muted-foreground/50'>
        <ShieldCheck className='h-3.5 w-3.5' />All your data is encrypted and secure
      </p>
    </div>
  );
}
...
function LinkExpiredCard({ emailFromUrl }: { emailFromUrl: string }) {
  return (
    <div className='w-full max-w-[440px] rounded-2xl border border-error/25 bg-[#1a0f0f] p-12 text-center shadow-xl shadow-black/40'>
      ...
      <Button asChild className='mt-8 w-full h-10 bg-primary hover:bg-primary-hover text-white font-semibold rounded-[--radius-md]'>
        <Link href={`/resend-verification?email=${encodeURIComponent(emailFromUrl)}`}>
...
function AutoVerify({ token, email }: { token: string; email: string }) {
  ...
  if (result === 'success') {
    return <CenteredShell><EmailVerifiedCard /></CenteredShell>;
  }
  if (result === 'error') {
    return <CenteredShell><LinkExpiredCard emailFromUrl={email} /></CenteredShell>;
  }
  return (
    <CenteredShell>
