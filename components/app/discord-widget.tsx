'use client';

import { useState, useEffect } from 'react';
import { X } from 'lucide-react';
import { cn } from '@/lib/utils';

const TOOLTIP_DISMISSED_KEY = 'postengage:discord-tooltip-dismissed';

function DiscordIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox='0 0 24 24'
      fill='currentColor'
      xmlns='http://www.w3.org/2000/svg'
    >
      <path d='M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z' />
    </svg>
  );
}

export function DiscordWidget() {
  const [showTooltip, setShowTooltip] = useState(false);

  const discordUrl = process.env.NEXT_PUBLIC_DISCORD_INVITE_URL || '#';

  useEffect(() => {
    if (typeof window !== 'undefined') {
      const wasDismissed = localStorage.getItem(TOOLTIP_DISMISSED_KEY);
      if (!wasDismissed) {
        // Show tooltip after a short delay for better UX
        const timer = setTimeout(() => setShowTooltip(true), 2000);
        return () => clearTimeout(timer);
      }
    }
  }, []);

  const dismissTooltip = () => {
    setShowTooltip(false);
    localStorage.setItem(TOOLTIP_DISMISSED_KEY, 'true');
  };

  return (
    <div className='fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2'>
      {/* Tooltip */}
      {showTooltip && (
        <div className='flex items-center gap-2 rounded-lg bg-card border shadow-lg px-3 py-2 animate-in fade-in slide-in-from-bottom-2 duration-300'>
          <p className='text-xs font-medium text-foreground whitespace-nowrap'>
            Need help? Join our Discord
          </p>
          <button
            onClick={dismissTooltip}
            aria-label='Dismiss tooltip'
            className='rounded p-0.5 hover:bg-muted transition-colors'
          >
            <X className='h-3 w-3 text-muted-foreground' />
          </button>
        </div>
      )}

      {/* Discord button */}
      <a
        href={discordUrl}
        target='_blank'
        rel='noopener noreferrer'
        onClick={dismissTooltip}
        aria-label='Join our Discord community'
        className={cn(
          'flex h-12 w-12 items-center justify-center rounded-full',
          'bg-[#5865F2] text-white shadow-lg',
          'hover:bg-[#4752C4] hover:scale-105',
          'active:scale-95',
          'transition-all duration-200',
          'discord-pulse'
        )}
      >
        <DiscordIcon className='h-6 w-6' />
      </a>

      {/* Pulse animation styles */}
      <style jsx>{`
        .discord-pulse {
          animation: discord-pulse-ring 2.5s cubic-bezier(0.4, 0, 0.6, 1)
            infinite;
        }
        @keyframes discord-pulse-ring {
          0%,
          100% {
            box-shadow: 0 0 0 0 rgba(88, 101, 242, 0.4);
          }
          50% {
            box-shadow: 0 0 0 8px rgba(88, 101, 242, 0);
          }
        }
      `}</style>
    </div>
  );
}
