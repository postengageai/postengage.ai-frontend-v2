...
class SocketService {
  ...
  // ─── Value analytics real-time events ────────────────────────────────────

  subscribeToWinsUpdate(
    callback: (item: {
      id: string;
      occurred_at: string;
      category:
    ...
    if (!this.socket) return;
    this.socket.on('wins:update', callback);
  }

  unsubscribeFromWinsUpdate(
    callback: (item: {
      id: string;
      occurred_at: string;
      category:
    ...
    if (!this.socket) return;
    this.socket.off('wins:update', callback);
  }

  subscribeToMilestoneAchieved(
    callback: (payload: {
      milestone_id: string;
      title: string;
      description: string;
    ...
    if (!this.socket) return;
    this.socket.on('milestone:achieved', callback);
  }

  unsubscribeFromMilestoneAchieved(
    callback: (payload: {
      milestone_id: string;
      title: string;
      description: string;
    ...
    if (!this.socket) return;
    this.socket.off('milestone:achieved', callback);
  }
